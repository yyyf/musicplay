...
function span_musictime_load(span_interval) {
	if(music_audio.readyState == 4){
		...
		let music_times_2 = Math.floor(music_times/60)+':'+music_times%60;

		if(Math.floor(music_times/60)<10)　music_times_2 = '0'+music_times_2;

		document.getElementById('music_length').innerHTML = music_times_2;
...
function span_musicnowtime_load(){
		...
		if(Math.floor(music_now_time_1/60)<10){music_now_time_1 = '0'+music_now_time_1;}
		if(Math.floor(music_now_time_2%60)<10){music_now_time_2 = '0'+music_now_time_2;}
		document.getElementById('music_surplus').innerHTML = music_now_time_1+':'+music_now_time_2;
		console.log(music_now_time);
}